...
def MappingProcess(keyframes_list, logger, cfg, newer_imgs, first_colmap_loop, lock, SEQUENTIAL_OVERLAP, adjacency_matrix, keypoints, descriptors, laf):
            ...
            if cfg.LOOP_CLOSURE_DETECTION == False:
                ...
                for l, m in zip(true_indices[0], true_indices[1]):
                    if l > m and l > old_adjacency_matrix_shape - 1:
                        for i in range(10):
                            print('PROVAAAA\n')
                        print('len(keyframes_list.keyframes())', len(keyframes_list.keyframes()))
                        for obj in keyframes_list.keyframes():
                            print(obj._keyframe_name)
                            print(obj._keyframe_id)
                        obj_buttare = keyframes_list.get_keyframe_by_id(4)
                        print(obj_buttare._keyframe_name)
